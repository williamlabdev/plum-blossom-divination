...
export function analyzeAt(chart: NajiaChart, frame: TimeFrame, qt: QuestionType, intent: AskIntent = '吉凶'): LiuyaoReport {
  ...
  // **比原文寬**；現收緊為死。
  //
  // 一個已知的分歧，動之前請先讀（0816 晚考據，全文見 CLAUDE.md「四時歸屬考據」）：
  // 訣列舉的四組只覆蓋八個月——野鶴〈四時旺相章第又十五〉的春＝正二月、夏＝四五月、
  // 秋＝七八月、冬＝十與十一月，**辰未戌丑四個土月不在訣裡**（該章明文「辰土旺…未土旺…
  // 戌土旺…丑土旺」，土月各有其令，故 `wangShuaiOf` 是對的，不要改成併入前一季）。
  // 引擎把訣外推到土月（土令 → 水為死 → 真空），**這一步超出原文**：全部真空判定中
  // 35.5% 來自土月。維持外推是選擇不是查證結果——訣可讀成枚舉（土月無真空），
  // 也可讀成舉例（底層是「月令所剋」，七言句塞不下第五組）。原文兩讀皆通。
  ...
  //
  //    一個已知分歧（0816 晚考據，全文見 CLAUDE.md「四時歸屬考據」）：訣列舉的四組只覆蓋
  //    八個月，辰未戌丑四個土月不在其中；引擎把它外推到土月（水為死→真空）超出原文。
  //    「改成併入前一季」那條**已結案不做**——〈四時旺相章第又十五〉明文土月各有其令。
  //    要收緊成「土月不入真空」的話，**這裡與上面的 `zhenKong` 必須一起改**（兩套判定）。
  if (yKong && !isBing) {
