...
// 進神／退神：動爻化出同五行而地支順進為「進神」、逆退為「退神」。
//
// 這份表照抄《增刪卜易・進神退神章第二十九》的原文列舉，一組不多一組不少：
//   進神  亥化子  寅化卯  巳化午  申化酉  丑化辰  辰化未  未化戌
//   退神  子化亥  卯化寅  午化巳  酉化申  辰化丑  未化辰  戌化未
//
// **土支在列，而且只有三組。** 此處原本寫「古法僅論四組，辰戌丑未土支不在其列」，
// 那句話是錯的——同一章底下的兩則案例（校準 #8、#10）用的正是上爻戌化未。
// 土的遞進是 丑→辰→未→戌 的**線性**序，原文沒有「戌化丑」「丑化戌」，
// 不要為了湊成循環而自行補上：四墓庫的順逆本就不是一個圈。
const JINSHEN_PAIRS: [Branch, Branch][] = [
  ['亥', '子'], ['寅', '卯'], ['巳', '午'], ['申', '酉'],
  ['丑', '辰'], ['辰', '未'], ['未', '戌'],
]
